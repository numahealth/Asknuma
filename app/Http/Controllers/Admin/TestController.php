        ...
        if ($condition == 'search') {
            ...
            Excel::load($path, function ($reader) {
                ...
                foreach ($reader->toArray() as $row) {
                    ...
                    if (isset($row['symptom_name']) && $row['symptom_name'] != null && $row['symptom_name'] != '') {
                        $key_array = array();
                        $symptom = trim($row['symptom_name']);
                        $symptoms = DB::table('symptom')
                                        ->select('symptom_name', 'id')
                                        ->where('symptom_name', '=', $symptom)->get();
                        if (empty($symptoms)) {
                        ...
                        foreach ($name_keyword as $value_symptom) {
                            $value_symptom = trim($value_symptom);
            ...
            Excel::load($path, function ($reader) {
                ...
                foreach ($reader->toArray() as $row) {
                    ...
                    if (isset($row['diseases_name']) && $row['diseases_name'] != null && $row['diseases_name'] != '') {
                        $symptom_array = array();
                        $deseases = trim($row['diseases_name']);
                        $symptoms = DB::table('diseases')
                                        ->select('disease_name', 'id')
                                        ->where('disease_name', '=', $deseases)->get();
                        if (empty($symptoms)) {
                ...
                foreach ($diseases_array as $key => $records) {
                    ...
                    foreach ($records as $symptom_id) {
                        DB::table('group')->insert(
                                ['name' => $row['group_name'], 'dieases' => $key,
                                    'status' => $row['status'], 'symptom' => $symptom_id,
                                    'mapping' => json_encode($records),
